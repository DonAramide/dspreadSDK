...
public class TLVParser {
    ...
    **/
    ...
    public static TLV searchTLV(List<TLV> tlvList, String targetTag) {
        Timber.d("Searching for tag: $targetTag");
        for (int i = 0; i < tlvList.size(); ++i) {
            Timber.d("Checking tag: " + tlvList.get(i).tag);
            TLV tlv = tlvList.get(i);
            if (tlv.tag.equalsIgnoreCase(targetTag)) {
                Timber.d("Found tag: " + targetTag);
                return tlv;
            } else if (tlv.isNested) {
                Timber.d("Nested tag found: ${tlv.tag}");
                TLV searchChild = searchTLV(tlv.tlvList, targetTag);
                if (searchChild != null) {
                    Timber.d("Found nested tag: $targetTag");
                    return searchChild;
                }
            }
        }
